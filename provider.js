...
class SequelizeProvider extends SettingProvider {
  ...
  async set(guild, key, val) {
    ...
    await this.model.upsert(
      {guild: guild === 'global' ? '0' : guild, settings: JSON.stringify(settings)},
      {where: {guild: guild === 'global' ? '0' : guild}}
    );
    if (guild === 'global') {
  ...
  async clear(guild) {
    ...
    await this.model.destroy({where: {guild: guild === 'global' ? '0' : guild}});
  }
  ...
  updateOtherShards(key, val) {
    ...
    val = typeof val === 'undefined' ? 'undefined' : JSON.stringify(val);
